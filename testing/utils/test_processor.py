...
from testing.utils.test_clean import clean_after_cmd
from testing.utils.set_test_window import set_test_window
...
def init_config(direpa_script):
    ...
    conf=dict(
        ...
        clean_after=cf["test"]["clean_after"],

        diren_src=cf["diren_src"],
        diren_test=cf["test"]["diren"],

        direpa_app=os.path.dirname(direpa_script),
        direpa_cmds=os.path.join(cf["test"]["direpa_root"], cf["test"]["filen_cmds"]),
        direpa_testgf=cf["test"]["direpa_root"],
        direpa_test=os.path.join(cf["test"]["direpa_root"], cf["test"]["diren"]),
        direpa_test_src=os.path.join(cf["test"]["direpa_root"], cf["test"]["diren"],cf["diren_src"]),
        ...
        filenpa_expect=os.path.join(direpa_script, "utils", cf["test"]['filen_expect']),
        filenpa_read_log_bashrc=os.path.join(cf["test"]["direpa_root"], cf["test"]["filen_read_log_bashrc"]),
        ...
        read_log_window_title=cf["test"]["read_log_window_title"],
        remote=cf["test"]["remote"],

        txt_screen_cmd_error=cf["test"]["txt_screen_cmd_error"],
        txt_screen_log_eof=cf["test"]["txt_screen_log_eof"],

        user_current=getpass.getuser(),

        waiting_time_between_cmds=cf["test"]["waiting_time_between_cmds"],
    )
    ...
    th.kill_screen_session(conf["main_session_name"])
    ...
    main_window_hex_id=th.get_active_window_hex_id()
    th.window_set_above(main_window_hex_id)
        ...
        for session_item in shell.cmd_get_value("screen -ls").splitlines():
            if conf["main_session_name"] in session_item:
                if "Attached" in session_item:
                    ...
                    # clear scrolling
                    th.send_cmd_to_screen(conf["main_session_name"], "echo -en \"\ec\e[3J\"")
                    
                    # focus_back on previous windows
                    # th.window_focus(launching_window_hex_id)
                    th.window_unset_above(main_window_hex_id)
                    th.window_focus(main_window_hex_id)
                    time.sleep(.5)
                    th.send_cmd_to_screen(conf["main_session_name"], "rm "+tmp_file)
                    break
...
def type_with_screen(line, test_name, conf):
    user_type=line.replace("type:", "").strip()
    raw_screen_ls_output=shell.cmd_get_value("screen -ls")
    session_name=""
    for raw_line in raw_screen_ls_output.splitlines():
        if test_name in raw_line:
            session_name=re.sub(r"(^.*\.#"+test_name+"#).*", r"\1", raw_line.strip())
            break
    
    if session_name:
        shell.cmd("screen -S "+session_name+" -p 0 -X stuff '"+user_type+"^M'")
    else:
        msg.app_error("No gnu screen session found for '"+test_name+"'")
        sys.exit(1)
...
def test_processor(conf):
    ...
    try:
        ...
        set_conf_json(conf)
        set_test_window(conf, test_name)
        ...
        for line_obj in conf["tmp"]["cmds_to_monitor"]:
            ...
            if key == "fail":
                ...
                tmp_tail_obj=tail_file_for_value(conf, tail_obj)
                ...
                th.send_cmd_to_screen(conf["main_session_name"], obj_value.replace("type:", "").strip())
                continue
            elif key == "step":
                if not obj_value:
                   obj_value="" 
        ...
        if tail_obj["interrupted"]:
            th.log(conf)
            th.kill_screen_session(conf[test_name]["session_name"])
            sys.exit(1)
        else:
                ...
                if block:
                    if "cmd_vars" in conf["tmp"]:
                        if block.group(1) in conf["tmp"]["cmd_vars"]:
                            ...
                            msg.user_error("'"+block+"' has no related key from set_test_vars",
                                "Line:"+str(count_line)+" "+caller_filename)
                            sys.exit(1)
                    else:
                        msg.user_error("'"+block+"' because set_test_vars is not set",
                            "Line:"+str(count_line)+" "+caller_filename)
                        sys.exit(1)
                else:
...
def set_test_vars(conf, var_obj):
    frame,caller_filename,caller_line_number,function_name,lines,index=inspect.stack()[1]
    if not "cmd_vars" in conf["tmp"]:
        conf["tmp"]["cmd_vars"]={}
    ...
    for key in var_obj:
        if key in  ["cmd","step", "info", "dep"]:
            msg.user_error(
                ...
                "key: '"+key+"' is a reserved keyword for test_processor.",
