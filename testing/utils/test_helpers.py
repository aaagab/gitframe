...
import shutil
from testing.utils.read_log import read_log
import uuid
import time
import stat
...
def expect_cmd(filenpa_expect, dict_cmd, password=""):
    DEVNULL = open(os.devnull, 'w')
    
    if not os.path.exists(filenpa_expect):
        msg.app_error(filenpa_expect+" not found!")
        sys.exit(1)        
    ...
    if password:
        try:
            ...
            fifo="/tmp/"+uuid_value
            subprocess.call(shlex.split("/bin/bash -c \"{ mkfifo "+fifo+" &> /dev/null &&  echo \""+sudo_pass+"\" > "+fifo+" & } &> /dev/null\""))
            subprocess.call(shlex.split(filenpa_expect+" '{}' '{}' '{}' '{}' '{}'".format(
                "true",
                dict_cmd["name"],
                dict_cmd["params"],
                dict_cmd["prompt"],
            ...
            # extra delete statement (already one in expect script, password cannot stay in tmp)
            subprocess.call(shlex.split("rm "+fifo), stdout=DEVNULL, stderr=DEVNULL)
        except:
            # extra delete statement (already one in expect script, password cannot stay in tmp)
            subprocess.call(shlex.split("rm "+fifo), stdout=DEVNULL, stderr=DEVNULL)
            msg.app_error("expect command '{}' with pass failed.".format(dict_cmd["name"]))
            sys.exit(1)
    else:
        try:
            subprocess.call(shlex.split(filenpa_expect+" '{}' '{}' '{}' '{}' '{}'".format(
            ...
            msg.app_error("expect command '{}' failed.".format(dict_cmd["name"]))
            sys.exit(1)
