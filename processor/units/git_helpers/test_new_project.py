...
if __name__ == "__main__":
    ...
    if sys.argv[1] == "new_project":
        if sys.argv[2] == "empty_path_exists":
            ...
            conf=Json_config().data
            abs_path=os.path.join(conf["processor"]["task"]["direpa_root"], "marty", "future")
            new_project(abs_path)
        elif sys.argv[2] == "relative_path_not_exist":
            conf=Json_config().data
            # print(conf["processor"]["task"]["direpa_root"])
            # abs_path=os.path.join(conf["processor"]["task"]["direpa_root"], "draft")
            new_project("test")      
    elif sys.argv[1] == "test_path_is_not_file":
        from git_helpers.new_project import test_path_is_not_file
        if sys.argv[2] == "file":
            test_path_is_not_file(os.path.join(conf["processor"]["task"]["direpa_root"], "file.txt"))      
        elif sys.argv[2] == "directory":
            test_path_is_not_file(conf["processor"]["task"]["direpa_root"])      
        ...
        create_directory(os.path.join(conf["processor"]["task"]["direpa_root"],"draft"))
    elif sys.argv[1] == "test_file_not_exist":
        from utils.json_config import Json_config
        conf=Json_config().data
        
        from git_helpers.new_project import test_file_not_exist
        test_file_not_exist(os.path.join(conf["processor"]["task"]["direpa_root"],"draft.txt"))
