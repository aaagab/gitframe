...
def tag(
    ...
    if dev_branch == principal_branch:
        msg.error(f"Tag can't be set on principal branch '{dev_branch}'.", exit=1)
    ...
    commands=[
        ...
        f'git push origin {principal_branch}',
    ...
    try:
        ...
        git.set_annotated_tags(f"v{version}", f"Bump release version {version}", remote_names=["origin"])
        commands.pop(0)
        commands.pop(0)
        ...
        commands.pop(0)
        git.push(remote_name="origin", branch_name=principal_branch)
        commands.pop(0)

        git.checkout(dev_branch)
        commands.pop(0)
        git.push(remote_name="origin", branch_name=dev_branch)
        commands.pop(0)
    except:
        has_error=True
        raise
